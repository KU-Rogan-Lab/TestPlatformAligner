...
def findAnchorPoints(img, visualize, low, high):
        # Finds the cordinates of the anchor objects (screws, stickers, etc) that are used to find the transform and base coords off of
        # Makes some assumptions:
        # - there are exactly 4 anchor objects, which all lie in a plane and are arranged on the corners of a square in that plane
...
def findLaserPoint(img, visualize, threshold): 
        ...
        for c in contours:
            M = cv.moments(c)
            if 0 < M["m00"] and M["m00"] < 400: # The laser dot contour will not be too big or too small, so look for that
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
